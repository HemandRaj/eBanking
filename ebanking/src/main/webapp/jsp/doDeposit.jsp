...
<title>OnlineBank_doDeposit</title>
...
<div id="ddblueblockmenu">
 ...
 <% String uname=(String)session.getAttribute("uname"); %>
    ...
    <li><a href="jsp/userep.jsp">Report</a></li>
        ...
        <%try{
			...
			while(rs.next()){
				...
				double bal=rs.getDouble("bal");
		         ...
		         {
		        	 double sum=bal+amt;
		        	 int s=st.executeUpdate("insert into trans(acc,credit,debit,bal,date) values ('"+accno+"','"+amt+"',0,'"+sum+"','"+date+"')");
		        	 int y=st.executeUpdate("update b_acc set bal='"+sum+"'where acc_no='"+accno+"'");
		        	 ResultSet r=st.executeQuery("select bal from b_acc where acc_no='"+accno+"'");
		        	 while(r.next()){
		        		 double balance=r.getDouble("bal");%>
		        	 <center><div class="box2">
	                <p style="color: green;">  Rs. <%=amt %> is successfull deposited in the account<%=accno %><br>
	                    Current Account Balance :Rs.<%=balance %></p>
	                  </div></center>
		        <%  }
		        }
		         else
